...
{
    public class UsgsDataFetcher : FloodzillaAzureJob
    {
        public static async Task FetchUsgsReadings(ILogger log)
        {
            string listenerInfo = "FloodzillaJob.FetchUsgsReadings, 5/2021, " + Environment.MachineName;

            JobRunLog runLog = new JobRunLog("FloodzillaJob.FetchUsgsReadings");
            ...
            try
            {
                ...
                using (SqlConnection sqlcn = new SqlConnection(FzConfig.Config[FzConfig.Keys.SqlConnectionString]))
                {
                    ...
                    runLog.Summary = String.Format("Checked {0} sites. {1} had errors. Received {2} new readings",
                                                   deviceCount, errorCount, readingCount);
                    runLog.ReportJobRunSuccess();

                    await SaveBlobText(container, "usgs-data-fetcher-details", sbDetails.ToString());
                    await sqlcn.CloseAsync();
                }
            }
            catch (Exception ex)
            {
                ErrorManager.ReportException(ErrorSeverity.Major, "FloodzillaJob.FetchUsgsReadings", ex);
