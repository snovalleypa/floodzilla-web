...
{
    public class DailyStatusJob
    {
        public static async Task SendDailyStatus(ILogger log)
        {
            JobRunLog runLog = new JobRunLog("FloodZillaMonitor.SendDailyStatus");
            try
            {
                using (SqlConnection sqlcn = new SqlConnection(FzConfig.Config[FzConfig.Keys.SqlConnectionString]))
                {
                    ...
                    runLog.Summary = String.Format("Sent daily status email about {0} locations", model.Statuses.Count);
